...
function Search() {
  ...
  // Filtrar proveedores basado en los filtros del store
  const filteredProviders = useMemo(() => {
    return allProviders.filter((provider) => {
      ...
      if (
        searchFilters.modality &&
        searchFilters.modality.length > 0 &&
        !searchFilters.modality.includes("all")
      ) {
        // Asumiendo que todos los proveedores tienen modalidad
      ...
      if (
        ...
        const doctor = provider as Doctor;
        const doctorGender = (doctor as any).gender;
      ...
      if (
        ...
        const hasMatchingLanguage = searchFilters.languages.some((lang) =>
      ...
      if (
        searchFilters.scheduledAppointments &&
        ...
        const hasMatchingSchedule = searchFilters.scheduledAppointments.some(
          (schedule) => providerSchedule.includes(schedule),
        );
      ...
      if (searchFilters.rating !== null && searchFilters.rating !== undefined) {
        ...
        if (!ratingArray.includes("all") && ratingArray.length > 0) {
          const minRating = Math.min(
            ...ratingArray.map((r) => Number(r)).filter((r) => !isNaN(r)),
          );
      ...
      if (
        ...
        const doctor = provider as Doctor;
        const doctorExperience = (doctor as any).yearsOfExperience;
        if (
          !doctorExperience ||
      ...
      if (
        searchFilters.insuranceAccepted &&
        ...
        const hasMatchingInsurance = searchFilters.insuranceAccepted.some(
          (insurance) => providerInsurance.includes(insurance),
        );
  ...
  // Opciones de filtros para desktop (actualizadas para coincidir con la data)
  const tipoProveedorOptions = [
    { value: "all", label: "Todos" },
    { value: "doctor", label: "Doctor" },
    { value: "hospital", label: "Hospital" },
    { value: "clinic", label: "Clínica" }, // Cambiado de "clinica" a "clinic"
  ];
